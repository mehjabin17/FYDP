...
df = pd.read_csv('./TextData/text_first_client.csv')
...
def train(net, trainloader, epochs: int):
    ...
    for epoch in range(epochs):
        ...
        for train_seq, train_mask, train_y in tqdm(trainloader):
          ...
          loss.backward()
          optimizer.step()
        print(f"For epoch 1 in client - 1 Accuracy: {correct_predictions/len(train_df)}, Loss: {np.mean(losses)}")
